...
1. 起動時（と「再スキャン」ボタン）で共有フォルダを再帰的にスキャンし、各ファイルの文字を抽出してディスクにキャッシュ（ファイルの更新日時で差分更新。変更のないファイルは読み直しません）
...
抽出したファイルの中身は `folder-search\.cache\index.json`（既定値）に保存されます。これにより:

- アプリを再起動しても、次回はキャッシュと突き合わせて**変更・追加されたファイルだけ**を読み直します（全社共有フォルダのようにファイル数が多い場合、初回スキャン以降はかなり速くなります）
- 保存先は `.env` の `CACHE_FILE` で変更できます（例: 別ドライブや高速なローカルSSD上に置く）
- **アプリや共有フォルダを別の場所へ移設**する場合は、`.cache` フォルダ（または `CACHE_FILE` が指すファイル）を新しい場所にそのままコピーしてください。ファイルの更新日時さえ保たれていれば（`robocopy` や通常のコピーでOK）、移設後も再スキャンなしで使えます
- キャッシュにはファイルの中身の抽出結果が入るため、Git 管理からは除外しています（`.gitignore` 済み）
