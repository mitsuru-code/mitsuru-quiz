...
【共通ルール】
- 情報源として共同通信（Kyodo News）は使わない。その他、政治的に偏向していると広く見なされている情報源も避け、一次情報や複数の独立したメディアで裏付けが取れる情報源を優先すること
...
該当する話題が「明確に」ある場合のみ breaking を true にしてください。迷ったら false（通常ニュース程度で乱発しない、ファン活動系ハッシュタグは対象外）。
...
以下のJSON形式のみで返答してください:
{
  ...
  // トレンド語で対象を絞っている分、オープンエンド探索より少ない検索回数で済む見込みだが、
  // 万一トレンドが的外れだった場合の再探索余地としてmaxSearchesは従来どおり2を維持
