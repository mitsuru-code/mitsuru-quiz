...
//           → 2時間後に正解をスレッド返信
//   20時 … 夜の振り返り: 1日の主要ニュースを振り返る5問クイズの長文ポスト
...
{
  "text": "投稿の全文（挨拶＋Q1〜Q20のQ&A＋締め＋出典）",
  "source": "使った主な出典メディアの一覧",
  "category": "朝ブリーフィング"
}`;
  return callClaude(prompt, 6000, 8);
}
...
それを元に、1日を振り返る「おさらいクイズ」を1本の長文ポストで作ってください:
- 冒頭に「🌙 今日も一日おつかれさまでした。今日のニュース、どれだけ覚えてる？振り返りクイズ5問です🐹」のような導入
- Q1〜Q5の5問。各問は選択肢2〜4個の択一式で、問題と選択肢を続けて書く
- 5問の下に「—— 正解はこちら ——」の区切りを入れ、A1〜A5として正解＋一言解説をまとめて記載（スクロールしないと答えが見えない構成）
- 最後に締めの一言と、主な出典を「（出典: NHK、Reuters ほか）」のようにまとめて記載

以下のJSON形式のみで返答してください:
{
  "text": "投稿の全文（導入＋Q1〜Q5＋区切り＋A1〜A5＋締め＋出典）",
  "source": "使った主な出典メディアの一覧",
  "category": "夜の振り返り"
}`;
  return callClaude(prompt, 4000, 8);
}
