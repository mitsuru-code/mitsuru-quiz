...
async function callClaudePlain(prompt, maxTokens, maxSearches) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    const markerIdx = clean.indexOf(META_MARKER);
    if (markerIdx === -1) {
      lastErr = new Error(`生成結果に${META_MARKER}区切りが見つかりません`);
      console.log(`⚠️ ${META_MARKER}区切りが見つかりません（試行${attempt}/2、stop_reason=${stopReason}）`);
      continue;
    }
    const text = clean.slice(0, markerIdx).trim();
    const metaBlock = clean.slice(markerIdx + META_MARKER.length);
...
最後に、投稿本文をそのまま出力し、その後に区切り行 ---META--- を1行入れ、続けて次の2行を書いてください（JSONにしない。本文中の引用符「"」はそのまま自由に使ってよい）:
---META---
