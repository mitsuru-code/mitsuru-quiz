...
test('scheduleFollowUps: followUp=falseなら何も積まない', () => {
  const state = {};
  scheduleFollowUps(state, 'h', 't', 1000, false, false);
  assert.strictEqual(state.breakingFollowUps, undefined);
});

test('scheduleFollowUps: followUp=true・uncertain=falseなら30分後の1件のみ', () => {
  const state = {};
  const now = 1_000_000;
  scheduleFollowUps(state, 'h', 't', now, true, false);
  assert.strictEqual(state.breakingFollowUps.length, 1);
  assert.strictEqual(state.breakingFollowUps[0].dueAt, now + 30 * 60000);
});

test('scheduleFollowUps: followUp=true・uncertain=trueなら30分/2時間/5時間後の3件', () => {
  const state = {};
  const now = 1_000_000;
  scheduleFollowUps(state, 'h', 't', now, true, true);
  assert.strictEqual(state.breakingFollowUps.length, 3);
  assert.deepStrictEqual(
    state.breakingFollowUps.map(f => f.dueAt),
    [now + 30 * 60000, now + 120 * 60000, now + 300 * 60000]
  );
});
