...
| **12時47分** | 昼のエンタメクイズ — 午前の出来事・スポーツ・芸能の明るい話題 | X投票（Poll）。2時間後に正解をスレッド返信 |
...
GitHub Actions（毎時起動。※GitHub都合で遅延・スキップされることがある）
   ...
   │
   ├─ 直近の投稿スロット（5時47分/12時47分/17時47分/20時47分）が未投稿なら
...
> 💡 **起動時刻を毎時47分にしている理由**: 毎時ちょうど0分はGitHub全体で最も混雑するタイミングで、起動そのものが間引かれやすいことが実測（平均108分間隔＝ほぼ半数の回が未起動）で分かりました。あえて0分・30分を避けて47分にすることで、起動の間引きを減らしています（朝のブリーフィングを5時台に早めるため、他のスロットも合わせて毎時47分に統一）。
