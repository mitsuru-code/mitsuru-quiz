...
GitHub Actions（毎時起動。※GitHub都合で遅延・スキップされることがある）
   ...
   │
   └─ 直近の投稿スロット（既定: JST 7時・12時・17時・21時）が未投稿なら
        Claude API（Web検索付き）でクイズ生成 → X API で投稿
        → 正解文を返信キュー（state.json）に登録（既定: 2時間後に返信）
```

> 💡 **キャッチアップ方式**: GitHubのcronは正確に毎時動く保証がないため、「起動時刻がスロットと一致したら投稿」ではなく「直近のスロットがまだ未投稿なら、遅れてでも投稿する」判定にしています。スロットから多少遅れることはありますが、取りこぼしは起きません（追いかけるのは直近1スロット分のみで、連投はしません）。
